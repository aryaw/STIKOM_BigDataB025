...
file_chart_svg = os.path.join(output_dir, f"forecast_substance_{fileTimeStamp}.html")
...
y = le_substance.fit_transform(df["substance_name"])
...
probs_df = pd.DataFrame(probs, columns=le_substance.classes_)
...
fig_svg.update_layout(
    title=f"Predicted Probability of Top 10 Substances Over Years (Including Forecast {forecast_year})",
    xaxis_title="Year",
